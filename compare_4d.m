...
filenames = filenames;
N = length(filenames);
queryIndex = xlsread('/home/ubuntu/Desktop/Thesis_Follow_Up_2/dmqRetrieval/lamdaDataset/qLabels_4d.xls');  % Reads randomly choosen query pairs from excell filequeryIndex = transpose( queryIndex ); 
%}
...
maxFront = 3;  
 ...
 for l = 1:240 % Number of Query Pairs , CAN TRY FOR DIFFERENT HAMMING RADIUS ALSO ?????? how?
    ...
    for j= 1:maxFront        
        ...
        Retrieved_Items_DMQIR{l,j}   =      pf_idx{l,j}(:,5); 
        predicted_labels_DMQIR{l,j}  =      targets(Retrieved_Items_DMQIR{l,j} , :); 
        
       % Retrieved_Items_EMR{l,j}   =      pf_idx_EMR{l,j}(:,5); 
