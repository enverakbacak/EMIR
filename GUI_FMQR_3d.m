function varargout = GUI_FMQR_3d(varargin)
% GUI_FMQR_3D MATLAB code for GUI_FMQR_3d.fig
%      GUI_FMQR_3D, by itself, creates a new GUI_FMQR_3D or raises the existing
%      singleton*.
%
%      H = GUI_FMQR_3D returns the handle to a new GUI_FMQR_3D or the handle to
%      the existing singleton*.
%
%      GUI_FMQR_3D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_FMQR_3D.M with the given input arguments.
%
%      GUI_FMQR_3D('Property','Value',...) creates a new GUI_FMQR_3D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_FMQR_3d_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_FMQR_3d_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_FMQR_3d

% Last Modified by GUIDE v2.5 04-Jun-2020 15:17:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_FMQR_3d_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_FMQR_3d_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_FMQR_3d is made visible.
function GUI_FMQR_3d_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_FMQR_3d (see VARARGIN)

% Choose default command line output for GUI_FMQR_3d
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_FMQR_3d wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_FMQR_3d_OutputFcn(hObject, eventdata, handles) 
...
set(gca,'FontSize',10); hold on;view(3);
rotate3d on; 
hold on;
...
Cmn = (q1(:) + q2(:) + q3(:)).'/2;
plot3(q1 , q2, q3, 'p' , 'MarkerSize',15,'MarkerFaceColor',[1 0 1]);
plot3(Cmn(1), Cmn(2), Cmn(3), 'd', 'MarkerSize',15 , 'MarkerFaceColor',[1 0 0]);
%plot3(q1 , q2, q3, 'gd' , 'LineWidth', 20); 
%plot3(Cmn(1), Cmn(2), Cmn(3), 'ro' , 'LineWidth', 10);
